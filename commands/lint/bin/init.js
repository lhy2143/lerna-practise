...
function write(filename, filePath, typePath) {
    fs.promises
        ...
        .then((data) => {
            fs.writeFile(filePath, data.toString(), 'utf8', (err) => {
                if (err) {
...
function generateConfig(typePath) {
    ...
    pathList.forEach((path) => {
        util.promisify(fs.access)(`./${path}`, fs.constants.F_OK)
            .then(() => {
                //文件存在
...
function checkDependencies(typescript) {
    let result = [];
    ...
    log(
        result.length
            ...
            `* ${result.join(' ')}\n` +
            `To install it, you can run: npm install ${result.join(
                ' '
            )} -D`
            : ''
    );
...
async function initialize(options) {
    ...
    checkDependencies(options.typescript);
    bar.update(60);
    // }
